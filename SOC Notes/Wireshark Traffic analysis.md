...
The next filter shows the TCP Connect scan patterns in a capture file.
...
The next filter shows the TCP SYN scan patterns in a capture file.
`tcp.flags.syn==1 and tcp.flags.ack==0 and tcp.window_size <= 1024`
...
The next filter shows the UDP scan patterns in a capture file.
`icmp.type==3 and icmp.code==3`

> [!hint] Window Size
> El campo `tcp.window_size` en el filtro de Wireshark se usa porque representa el **tamaño de la ventana TCP**, que indica cuántos datos puede recibir un dispositivo antes de requerir una confirmación (**ACK**).
...
![[Pasted image 20250324164502.png]]
...
> [!hint] 
> `eth.dst == 00:0c:29:e2:18:b4 and http` Podremos filtrar trafico http hacia la direccion MAC seleccionada
...
![[Pasted image 20250324183639.png]]
...
![[Pasted image 20250324183828.png]]
...
An encryption key log file is a text file that contains unique key pairs to decrypt the encrypted traffic session. hese key pairs are automatically created (per session) when a connection is established with an SSL/TLS-enabled webpage.
You need to configure your system and use a suitable browser (Chrome and Firefox support this) to save these values as a key log file.
To do this, you will need to set up an environment variable and create the **SSLKEYLOGFILE**, and the browser will dump the keys to this file as you browse the web. SSL/TLS key pairs are created per session at the connection time, so it is important to dump the keys during the traffic capture.
`Edit --> Preferences --> Protocols --> TLS` menu to add/remove key log files.
...
 
![[Pasted image 20250402144717.png]]


#### Actionable Results!

You can create firewall rules by using the **"Tools --> Firewall ACL Rules"** menu.
...
![[Pasted image 20250402144728.png]]
